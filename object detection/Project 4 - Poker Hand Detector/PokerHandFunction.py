def findPokerHand(hand):
    ...
    suits = []
    possibleRanks = []
    # cards and their hand
    for card in hand:
        # if card len is 2 then rank is card[0] is A, K,... and suit is card[1] is H, D, C, S
        if len(card) == 2:
            rank = card[0]
            suit = card[1]
        # if card len is 3 then rank is card[0:2] is 10 and suit is card[2] is H, D, C, S
        else:
            rank = card[0:2]
            suit = card[2]
        # convert rank to int
